...
batch = grigio.reshape(1, grigio.shape[0], grigio.shape[1])
feature_vector = grigio.reshape(1, -1)

plt.imshow(grigio, cmap="gray")
plt.axis("off")
plt.savefig("foto_aula_grigio.png", bbox_inches="tight", pad_inches=0)
plt.close()

print(f"shape RGB: {foto.shape}")
print(f"shape grigio: {grigio.shape}")
print(f"shape batch: {batch.shape}")
print(f"shape feature vector: {feature_vector.shape}")
print(f"intervallo: {grigio.min():.2f} - {grigio.max():.2f}")
print("Immagine salvata: foto_aula_grigio.png")
